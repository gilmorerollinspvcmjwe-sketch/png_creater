...
- 最终给出友好的回复"""
    ...
    async def _run_v2(
        ...
        # ---- We have enough info: run Planner → Builder → Validator pipeline ----
        session.state = SessionState.GENERATING
        try:
            from .pipeline import run_pipeline

            async def llm_call_wrapper(messages, schema=None):
                """把项目 LLM 客户端适配给 Builder 文案/组件选择工具。"""
                from ..llm.client import Message
            ...
            preferred_theme = context.get("theme_id", "sakura") if context else "sakura"
            result = await run_pipeline(
                ...
                theme_id=preferred_theme,
            )

            config = result.get("config")
            if not config:
                raise ValueError(result.get("error") or "Planner/Builder/Validator 未生成配置")

            plan = result.get("plan", {})
            validation = result.get("validation_report", {})
            theme_id = plan.get("theme_id", preferred_theme)
            build_reasoning = result.get("build_reasoning", "已完成稳定生成链路")
            ...
            return AgentResponse(
                session_id=session.id,
                response=f"太好了！我按 Planner → Builder → Validator 稳定链路生成了一个{theme_id}风格主页 ✨ {build_reasoning}",
                action={
                    "type": "generate",
                    "data": {
                        "theme": theme_id,
                        "pipeline": "planner_builder_validator",
                        "plan": plan,
