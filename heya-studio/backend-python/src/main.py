    ...
    async def event_generator() -> AsyncGenerator[str, None]:
        ...
        async def run_agent():
            try:
                ...
                session_id = await _process_chat_request(request, workflow)
        ...
        final_session_id = session_id_container[0] if session_id_container else request.sessionId
...
async def list_templates(request: Request, style: Optional[str] = None, limit: int = 10):
    ...
    return {
        "templates": [t.model_dump() for t in result.templates],
        "total": result.total
    }
...
@limiter.limit("30/minute")
async def add_guestbook_message(http_request: Request, page_id: str, request: GuestbookMessageRequest):
    ...
    result = await tool.execute({
        ...
        "is_owner_reply": request.isOwnerReply,
