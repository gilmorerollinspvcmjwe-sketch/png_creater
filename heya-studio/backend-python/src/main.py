...
async def _run_design_agent_with_workflow(
    ...
    主路径只让 LLM 做理解和文案，配置由 Builder 工具生成，再由 Validator 兜底。
    """

    # 展示用户画像，帮助前端进度条讲清楚“正在理解什么”。
    ...
    workflow.add_step("thinking", "🧭 Planner：理解意图、主题和组件方向")
    workflow.add_step("acting", "🛠️ Builder：用工具生成合法主页配置")
    workflow.add_step("tool_call", "🔍 Validator：校验配置结构、语义和布局", {"tool": "validator"})

    # 当前 API 主路径强制启用 V2，避免回到完整 JSON 直出。
    previous_use_v2 = getattr(agent, "use_v2", False)
    agent.use_v2 = True
    try:
        response = await agent.run(message, session, context)
    finally:
        agent.use_v2 = previous_use_v2

    data = response.action.get("data", {}) if response.action else {}
    plan = data.get("plan", {})
    validation = data.get("validation", {})
    component_count = len(response.current_config.get("components", [])) if response.current_config else 0

    if plan:
        workflow.add_step("skill_match", f"🎨 Planner 选择 {plan.get('theme_id', 'sakura')} 风格", {
            "theme": plan.get("theme_id", "sakura"),
            "skill": plan.get("skill_id"),
        })
    if response.current_config:
        workflow.add_step("generating", f"✅ Builder 生成了 {component_count} 个组件", {
            "component_count": component_count,
        })
    if validation:
        workflow.add_step("validation", "✅ Validator 校验通过" if validation.get("passed") else "⚠️ Validator 发现问题", {
            "valid": validation.get("passed"),
